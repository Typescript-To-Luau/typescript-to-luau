export class Parser {
    ...
    parseStatemt() {
        ...
        if (statement.type === "Keyword" && statement.value === "function") {
            ...
            if (!nameT || nameT.type !== 'Identifier')
                throw new Error("Expected function names");
            ...
            while (this.peek() && this.peek().type !== "Symbol" && this.peek().value !== ")") {
                ...
                if (!paramT || paramT.type !== "Identifier")
            ...
            if (!openBody || openBody.type !== "Symbol" || openBody.value !== "{")
            ...
            if (!closeBody || closeBody.type !== "Symbol" || closeBody.value !== "}")
                throw new Error("Expected '}' to close function body");
        ...
        if (statement.type === "Keyword" && (statement.value === "const" || statement.value === "let" || statement.value === "var")) {
            ...
            const nameT = this.consume();
            if (!nameT || nameT.type !== "Identifier")
                throw new Error("Expected variable name after decleration keyword");
            const name = nameT.value.toString();
            const equals = this.consume();
            if (!equals || equals.type !== "Symbol" || equals.value !== "=")
